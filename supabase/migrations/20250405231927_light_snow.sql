/*
  ...
  1. Changes
    - Create a new public bucket for podcasts
    - Set up storage policies for admin-only uploads
    - Handle existing policies gracefully

  2. Security
    - Public can view/download podcast files
    - Only admin can upload/delete podcast files
*/
...
BEGIN
  ...
  DROP POLICY IF EXISTS "Podcast Files Admin Upload Access" ON storage.objects;
  DROP POLICY IF EXISTS "Podcast Files Admin Delete Access" ON storage.objects;
